...
const onSlideChange = (e) => {
	index = e.srcElement.swiper.activeIndex + 1; // ... lol
	...
	<Grid max={2} min={1}>
		<div class="relative w-full h-0 pb-[100%]">
			{#if product.images?.edges?.length > 0}
				<swiper-container bind:this={swiper} class="w-full h-[calc(100%_+_34px)] absolute top-0 left-0" slides-per-view={1} loop={true} on:slidechange={onSlideChange}>
					<div slot="container-end">
						{#if product.images?.edges?.length > 1}
							<div class="text-[var(--text-color)] absolute bottom-0 right-0 z-10">
								<button class="cursor-pointer" aria-label="Previous Slide">{@html "<"}</button>
								<span>
									{index}
									/
									{product.images?.edges?.length}</span>
								<button class="cursor-pointer" aria-label="Next Slide">{@html ">"}</button>
							</div>
						{/if}
					</div>
					{#each product.images.edges as image}
						<swiper-slide class="w-full h-[calc(100%_-_34px)] relative pointer-events-auto">
		...
		<div class="flex items-center">
			<div class="max-w-[350px] m-auto w-full">
				<div class="pt-[20px] flex ">
					<div class="flex-grow">
						<h1 class="h6 !mb-0 pr-[20px]">{product.title}</h1>
					</div>
